...
A production solver should also check boundary length, bus ordering,
differential pairs, and layer constraints.
...
| `connections` | Ordinary connections or differential pairs. Each endpoint identifies its region and position. The type includes optional `externalDestination`, but Core does not currently populate it. |
...
Return `{ breakoutPoints }` synchronously. Every point must provide `regionId`,
`connectionId`, `layer`, `x`, and `y` on the matching region boundary, with
exactly one point per `(regionId, connectionId)` pair; differential-pair
members count separately. Missing, unknown, or duplicate pairs and Promise
returns all cause layout to fail.
