...
PCB layout and must synchronously return the PCB-world positions and layers of
the generated breakout points.
...
The endpoint type also includes an optional `externalDestination`, representing
a PCB-world position beyond its breakout region. Core does not currently
populate this field, so solvers must not rely on it being present.
