...
[`<breakout />`](../elements/breakout.mdx) regions. The callback may be
synchronous or asynchronous and returns the PCB-world positions and layers of
...
region boundary. The callback can return the object directly or return a
`Promise` when it needs asynchronous computation.
