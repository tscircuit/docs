...
description: >-
  Use a custom PCB autorouter or implicit breakout-point solver directly from
  tscircuit code.
---
...
Use `implicitBreakoutPointSolverFn` when the normal autorouter should still
route traces, but your application needs to choose where connections leave
[`<breakout />`](../elements/breakout.mdx) regions. The callback runs during
PCB layout and must synchronously return the PCB-world positions
and layers of the generated breakout points.
...
export default () => (
  <board width="20mm" height="12mm">
    ...
    >
      {/* components whose traces leave this region */}
    </breakout>
  </board>
)
```

This intentionally small solver spaces points from the lower or left end of
each boundary. A production solver should also check the available boundary
length, preserve bus ordering, keep differential-pair members together, and
choose layers that satisfy the board's routing constraints.

The input contains:

| Field | Description |
| --- | --- |
| `regions` | Breakout IDs, rectangular bounds, and selected boundary edges. Coordinates are PCB world-space millimeters. |
| `connections` | Ordinary connections or differential pairs. Each endpoint identifies its region and position. |
| `buses` | Bus IDs, member connection IDs, and optional ordered target layers. |
| `boundaryPointSpacing` | Recommended spacing between generated boundary points, in millimeters. |

An endpoint can also include `externalDestination`, the PCB-world position
beyond its breakout region. Use it to prefer an edge nearer the connection's
eventual destination or to align related escapes.

Return `{ breakoutPoints }`. Every point must provide `regionId`,
`connectionId`, `layer`, `x`, and `y`. The point must lie on the corresponding
region boundary. Return exactly one point for every `(regionId, connectionId)`
pair requested by the input, counting the two members of a differential pair
separately. Unknown or duplicate pairs, as well as missing points, cause PCB
layout to fail. Core invokes this callback synchronously; returning a `Promise`
throws an error.
