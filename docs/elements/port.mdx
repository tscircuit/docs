...
| `schPinLabelFontSize` | `"default" \| "sm" \| distance` | Sets this port's schematic pin-label size. `"sm"` resolves to `0.12mm`; a positive distance sets an exact size; `"default"` preserves the renderer default. |

## Pin Label Font Size

Use `schPinLabelFontSize` when a long pin label needs smaller text without
scaling the symbol. The `"sm"` preset uses `0.12mm`. You can also provide a
positive distance such as `"0.1mm"`. Use `"default"` or omit the prop to keep
the renderer's normal label size.

This prop changes only the label text size; it does not change the port
position, stem length, or symbol size.
