...
respectively. Omit `schSize`, or use `"default"`, for the standard symbol.
...
| `schSize` | Selects the schematic symbol size. Use `"sm"` or `"xs"` for compact symbols; omit it or use `"default"` for the standard size. | `"sm"`, `"xs"` |
