...
| `schSize` | Schematic symbol size. `"sm"` and `"xs"` select compact premade symbols (`boxresistor_sm` / `boxresistor_xs`, or `resistor_sm` / `resistor_xs` when `symbolName="resistor"`). `"default"`, `"md"`, and numeric distances are accepted by the props type but currently keep the standard symbol. | `"sm"`, `"xs"` |

Compact `"sm"` and `"xs"` symbols shorten the pin-to-pin span (about `0.5` and `0.35` schematic units). They are only applied when the symbol is the default box resistor or the US-style `resistor` symbol.
...
export default () => (
  <board width="20mm" height="10mm">
    <resistor name="R_DEFAULT" resistance="1k" schX={0} schY={0} />
    <resistor name="R_SM" resistance="1k" schSize="sm" schX={3} schY={0} />
    <resistor name="R_XS" resistance="1k" schSize="xs" schX={6} schY={0} />
  </board>
)
`}
/>
