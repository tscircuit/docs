...
| `schSize` | Schematic symbol size. `"sm"` and `"xs"` select compact non-polarized `capacitor_sm` / `capacitor_xs` symbols. `"default"`, `"md"`, and numeric distances are accepted by the props type but currently keep the standard symbol. Polarized capacitors ignore compact sizes and keep `capacitor_polarized`. | `"sm"`, `"xs"` |
