...
Each direct child `<port />` is part of the group's public interface. Use
`connectsTo` to map it to one or more ports on components inside the group.
`direction` and `schPinArrangement` control schematic placement; they do not
define connectivity.

## Example: LED matrix module

Collapsed schematic boxes are useful when a group contains repetitive internal
circuits. This 3x3 LED matrix exposes row and column ports while hiding the
individual LEDs in the schematic. The example uses a row-anode, column-cathode
topology. To use the inverse topology, swap `anode` and `cathode` throughout.
  ...
  export default () => (
    <board width="56mm" height="28mm">
      ...
      <resistor
        ...
        pcbX={-7}
        pcbY={4}
        connections={{ pin1: "J_ROWS.ROW1", pin2: "MATRIX.ROW1" }}
      />
      <resistor
        name="R_ROW2"
        resistance="1k"
        ...
        schX={-4}
        pcbX={-7}
        connections={{ pin1: "J_ROWS.ROW2", pin2: "MATRIX.ROW2" }}
      />
      <resistor
        name="R_ROW3"
        resistance="1k"
        ...
        pcbX={-7}
        pcbY={-4}
        connections={{ pin1: "J_ROWS.ROW3", pin2: "MATRIX.ROW3" }}
      />
    </board>
  )
  `}
...
`pcbGrid` arranges the LEDs in three rows and three columns. The internal
`<trace />` elements define six shared nets, which the autorouter connects on
the PCB. At the board level, `J_ROWS` connects to the row ports through
current-limiting resistors, while `J_COLS` connects directly to the column
ports.
...
The `<group />`, connectors, and resistors are siblings within the top-level
`<board />`. Setting `name="MATRIX"` on the group makes its public ports
available as `MATRIX.ROW1`, `MATRIX.COL1`, and so on. The external components
use these `GROUP_NAME.PORT_NAME` selectors, so internal names such as `D1`
remain encapsulated.
