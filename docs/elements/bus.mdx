...
The `<bus />` element tells the autorouter that one or more connections belong
to the same group. Each entry in `connections` can be a named
[`<trace />`](./trace.mdx) or a
[port selector](../guides/tscircuit-essentials/port-and-net-selectors.md).
...
## Group a Single Connection

A bus may contain one connection. This is useful for control signals that
should retain bus-level routing and fanout intent even though they do not need
length-skew matching.

```tsx
<autoroutingphase
  autorouter="fanout"
  busFanoutDirections={{ RESET: "rightside_center" }}
/>

<trace name="RESET_n" from="U1.RESET" to="J1.RESET" />
<bus name="RESET" connections={["RESET_n"]} />
```

The bus name matches the `busFanoutDirections` key, so the singleton
`RESET_n` connection exits through the center region of the right boundary.
At least one connection is required; an empty `connections` array is invalid.
...
export default () => (
  ...
  >
    <autoroutingphase
      autorouter="fanout"
      busFanoutDirections={{ DATA: { direction: "rightside_center" } }}
...
| -------- | ----------- | ------- |
| `connections` | One or more trace names or port selectors in the bus. Empty arrays are invalid. | `["RESET_n"]` |
