...
A singleton bus can apply layer, width, or fanout constraints to one control
signal.

For recommendations on dividing DDR signals into buses and fanning them out of
opposing packages, see [Routing DDR](../guides/routing-ddr.mdx).
...
| -------- | ----------- | ------- |
| `connections` | One or more trace names or port selectors in the bus. | `["D0", "D1"]` |
...
## Singleton control net

A singleton bus can set `pcbTraceWidth` on one named RESET trace.
