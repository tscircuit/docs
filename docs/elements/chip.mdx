...
## KiCad Metadata Properties

When working with KiCad, chips support two metadata properties that preserve KiCad-specific information:

- `kicadFootprintMetadata` - Stores metadata from KiCad footprint files (`.kicad_mod`)
- `kicadSymbolMetadata` - Stores metadata from KiCad symbol files (`.kicad_sym`)

These properties are automatically populated when importing from KiCad and are used when exporting back to KiCad format. See [KiCad Metadata](/guides/kicad/kicad-metadata) for complete documentation of all available fields.
