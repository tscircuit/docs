-## Active-Low Pin Labels (Top Bar)

If you want a schematic pin label to render with a bar above it (commonly used
for active-low signals), prefix the `pinLabels` value with `N_`.

- `RESET` renders as a normal label
- `N_RESET` renders as `RESET` with a bar at the top
...
export default () => (
  <board width="30mm" height="20mm">
    <chip
      ...
      schPinArrangement={{
        leftSide: { direction: "top-to-bottom", pins: ["RESET"] },
        rightSide: { direction: "top-to-bottom", pins: ["N_RESET"] },
      }}
    />
  </board>
)
