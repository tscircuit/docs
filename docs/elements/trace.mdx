...
| `name` | Names the trace and renders as the schematic net label for direct port-to-port traces (optional) | `"SENSE"` |
...
`maxLengthSkew`. The routed lengths and tolerance are measured in millimeters,
not as a ratio. This only constrains routed length; it does not set pair spacing
or differential impedance. See the dedicated page for complete trace-name and
pin-selector examples.
