...
You can also create a manual trace path without a `to` endpoint by using `path`
with a single selector. This is useful for RF feeds, antenna traces, and other
open-ended copper paths where the physical PCB geometry is the important part
of the design.
