...
:::note `displayName` is not rendered yet
`displayName` is accepted by the props API, but it cannot yet replace `name` in
the schematic renderer because `schematic_component` does not have a
`display_name` field. The symbol currently displays its `name`.
:::
...
| `displayName` | `string` | No | Reserved human-facing name. Currently accepted but not rendered. |
