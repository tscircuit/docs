...
`automaticPoursEnabled` defaults to `false`. When routing is enabled, it adds
implicit pours after routing for nets marked or recognized as power or ground.
It does not suppress traces, reserve layers, or terminate package pins on a
plane; use explicit `<copperpour />` elements when those details need control.
