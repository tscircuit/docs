...
The `<board />` element contains all the chips and traces to create a PCB.
Use it as the root element for a single board, or place multiple boards inside a
[`<panel />`](./panel.mdx) for panelization.
...
| `layers` | `1 \| 2 \| 4 \| 6 \| 8 \| 10` | Specify the number of copper layers in the board stackup. Defaults to 2 layers. |
| `allowBlindAndBuriedVias` | `boolean` | Allow partial-stack autorouter vias. Defaults to `false` (full-stack vias); confirm manufacturer support before enabling. |
| `autorouter` | `'auto' \| 'auto-local' \| 'fanout' \| 'laser_prefab' \| 'auto_jumper' \| AutorouterConfig` | Select a built-in autorouter preset or provide a configuration object. |
...
| `autorouterVersion` | `'beta_pipeline1' \| 'beta_pipeline3' \| 'beta_pipeline4' \| 'beta_pipeline5' \| 'beta_pipeline7' \| 'beta_pipeline9' \| 'latest'` | Pin routing to a known autorouter implementation when comparing routing behavior across versions. Unknown values fall back to `latest` with a warning. |
| `routingDisabled` | `boolean` | Skip routing to speed up development. |
| `routeRemaining` | `boolean` | Set to `false` to skip implicit routing of remaining connections while keeping explicit routing phases and breakouts. Unrouted connections still produce DRC errors. `true` or omission keeps normal remaining routing; nested subcircuits inherit the setting. |
...
| `outline` | `BoardOutlinePoint[]` | Supply a custom polygon. A castellated point requires `isCastellatedHole`, `holeDiameter`, and `padDiameter`; `connectsTo` is optional. |
...
| `automaticPoursEnabled` | `boolean` | After routing, generate implicit copper pours for nets marked or recognized as power or ground. Defaults to `false`; use `<copperpour />` when layer, net, outline, or clearance must be explicit. |
...
- `autorouter="auto"` - Uses the recommended router from the [platform configuration](../guides/running-tscircuit/platform-configuration.md).
- `autorouter="auto-local"` - Use the platform configuration, but only route locally (do not make API calls)
- `autorouter="fanout"` - Escape dense packages before routing the remaining connections. Use it with [`fanoutPourNetMap`](./autoroutingphase.mdx#terminate-power-connections-on-copper-planes) when package power pins should terminate on dedicated copper planes.
- `autorouter="bus_lanes"` - Route selected bus connections without adding vias. Prefer a [bus-lanes routing phase](./autoroutingphase.mdx#route-bus-lanes-without-layer-changes) so remaining connections can use the global router.
...
### Generating Automatic Copper Pours

Set `automaticPoursEnabled` when you want tscircuit to add copper coverage for
power and ground nets without declaring each region manually. A net qualifies
when it is marked with `isPowerNet` or `isGroundNet`, or when its name matches
tscircuit's built-in power or ground naming rules.

```tsx
<board width="30mm" height="20mm" layers={4} automaticPoursEnabled>
  <net name="GND" isGroundNet />
  <net name="V3_3" isPowerNet />

  <chip name="U1" footprint="qfn32" />
  <trace from=".U1 > .pin1" to="net.GND" />
  <trace from=".U1 > .pin2" to="net.V3_3" />
</board>
```

The option defaults to `false` and is skipped when PCB routing is disabled.
When enabled, tscircuit chooses the pour regions and layers after PCB traces
have been routed and before the final PCB design-rule checks. The pours flow
around existing pads, traces, cutouts, and keepouts. Enabling the option does
not reserve a layer for a net, suppress that net's traces, or tell the
autorouter that a pin should terminate on a plane.

Use an explicit [`<copperpour />`](./copperpour.mdx) when the poured layer, net,
outline, or clearance must be predictable. Avoid overlapping an automatic pour
with an explicit pour for the same region unless the overlap is intentional.

For a dense package whose power pins should drop directly onto dedicated inner
planes, use a fanout routing phase with `fanoutPourNetMap` and explicit pours:
...
See [`<autoroutingphase />`](./autoroutingphase.mdx#terminate-power-connections-on-copper-planes)
for the complete plane-termination behavior.
