...
### Draw a Fixed-Size Box

For a fixed-size box, provide both `width` and `height`. The box is centered at
`schX` and `schY`.
...
### Titles and Outline Style

Use `titleAlignment` to anchor the title to any of the nine box positions, such
as `top_left`, `center`, or `bottom_right`. `titleInside` controls whether the
title sits inside or outside the box. Set `strokeStyle` to `solid` or `dashed`.
