...
For a complete bus-fanout workflow with opposing controller and memory
breakouts, layer planning, skew constraints, and verification, see
...
## Reusing pre-generated routes

[`<fanout />`](./fanout.mdx) is an alias of `<breakout>`. Pass `pcbTracePaths` to either element
to reuse saved port-to-exit routes, including routes that start or end with vias.
Core creates the exits and preserves the supplied copper during global routing.
See [Reusing saved fanout trace paths](../guides/reusing-saved-fanout-trace-paths.mdx)
for a complete example, JSON imports, coordinate rules, and coverage requirements.
...
| `pcbTracePaths` | Saved wire/via routes keyed by port selector. Replaces automatic routing for this fanout and must cover all its routing connections. |
