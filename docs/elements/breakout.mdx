...
## Choose Exact Fanout Exit Edges

Use edge-first `busFanoutDirections` values when the physical exit edge
matters. The prefix before `side` names the boundary edge, while the suffix
always names the region along that edge. A corner suffix also names the local
direction used to escape the source pads. For a `*_center` value, the pads
instead escape toward the physical edge named by the prefix.

For example, `rightside_top` terminates on the right edge in its upper region
after escaping the source pads toward the top. `topside_right` instead
terminates on the top edge in its right region after escaping toward the
right.

```tsx
<breakout
  busFanoutDirections={{
    CONTROL: "rightside_top",
    CLOCKS: { direction: "topside_right" },
  }}
>
  {/* Components, buses, and traces */}
</breakout>
```

| Physical edge | Canonical values |
| --- | --- |
...
Directions use board world coordinates: right is positive X and top is
positive Y. They do not rotate with the source component. Legacy nine-point
values such as `center_right` and `top_left` remain accepted and keep their
destination-guided behavior, but edge-first values are preferable when the
exit edge must be unambiguous.

## Keep Space Between Fanout Boundaries

Set `fanoutMargin` to require clearance between sibling breakout boundaries.
It accepts a nonnegative distance; raw numbers are millimeters.

```tsx
<breakout name="LEFT" pcbX="-4mm" fanoutMargin="0.6mm">
  {/* Left fanout */}
</breakout>
<breakout name="RIGHT" pcbX="4mm" fanoutMargin="0.3mm">
  {/* Right fanout */}
</breakout>
```

When two sibling breakouts specify different margins, tscircuit requires the
larger value, so the example requires at least `0.6mm` between their
boundaries. Fanout boundaries may never overlap, even when `fanoutMargin` is
omitted or zero.

`fanoutMargin` controls clearance between separate fanout boundaries.
`fanoutBoundaryPadding` instead controls the distance from a fanout's source
pads to its own shared termination boundary.
...
| `busFanoutDirections` | Maps named buses to fanout directions and boundary regions. Prefer edge-first values when the physical exit edge matters. |
| `fanoutBoundaryPadding` | Padding between the source pads and the shared boundary where fanout traces terminate. |
| `fanoutMargin` | Nonnegative minimum clearance between sibling fanout boundaries. Raw numbers are millimeters; the larger margin in a pair is used. |
