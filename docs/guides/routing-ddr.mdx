...
export const controllerByteLaneFanoutCode = `const laneSignals = [
  "DQ0",
  "DQ1",
  "DQS0_t",
  "DQS0_c",
  ...
  >
    ...
    >
      ...
      {laneSignals.map((signal, index) => (
        <breakoutpoint
          key={signal}
          connection={".U_CONTROLLER > ." + signal}
          pcbX={5.9999}
          pcbY={fanoutExitYs[index]}
        />
      ))}
    </breakout>
...
export const memoryByteLaneFanoutCode = controllerByteLaneFanoutCode
  .replace('name="CONTROLLER_FANOUT"', 'name="MEMORY_FANOUT"')
  .replaceAll("U_CONTROLLER", "U_MEMORY")
  .replace("rightside_center", "leftside_center")
  .replace("pcbX={-4}", "pcbX={4}")
  .replace("pcbX={5.9999}", "pcbX={-5.9999}")
  .replace("pcbX={6}", "pcbX={-6}")
...
The first preview isolates a controller byte-lane fanout. The small pads on the
right stand in for the global channel so the local escape is easy to inspect.
They are outside the breakout; explicit [`<breakoutpoint />`](../elements/breakoutpoint.mdx)
elements hold the shared boundary between the local fanout and the global
channel route.

<CircuitPreview
  verticalStack
  defaultView="pcb"
  code={controllerByteLaneFanoutCode}
/>

The memory side mirrors the physical exit edge. Its channel boundary is on the
left, facing the controller.

<CircuitPreview
  verticalStack
  defaultView="pcb"
  code={memoryByteLaneFanoutCode}
/>

In a complete design, put the actual packages in separate breakouts and define
the buses and point-to-point traces on the enclosing board:
