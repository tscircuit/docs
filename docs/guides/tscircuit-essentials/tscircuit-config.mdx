...
- `kicadProject` (`boolean`): Enable KiCad project output (`.kicad_pro`, `.kicad_sch`, `.kicad_pcb`) in build. Equivalent to `--kicad-project` CLI flag.
- `kicadLibrary` (`boolean`): Enable KiCad library output in build. Equivalent to `--kicad-library` CLI flag.
- `kicadPcm` (`boolean`): Enable KiCad Plugin/Content Manager assets output in build. Equivalent to `--kicad-pcm` CLI flag.
- `previewImages` (`boolean`): Enable preview image outputs in build. Equivalent to `--preview-images` CLI flag.
- `glbs` (`boolean`): Enable GLB 3D model outputs for each circuit in build. Equivalent to `--glbs` CLI flag.
- `typescriptLibrary` (`boolean`): Enable TypeScript library output in build. Equivalent to `--transpile` CLI flag.
