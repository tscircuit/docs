...
| `useCloudAutorouting` | Enable cloud autorouting for CLI builds, exports, and snapshots. |
...
Entry file for KiCad project generation (`.kicad_pro`, `.kicad_sch`, `.kicad_pcb` files).

**Use Cases:**

- **KiCad project exports:** Define which file is used to generate KiCad project files when running `tsci build --kicad-project` without specifying a file argument.

**Example:**

```json
{
  "kicadProjectEntrypointPath": "lib/my-circuit.tsx"
}
```

**How It Works:**

- When `build.kicadProject` is `true` or `--kicad-project` flag is used, this file is built and converted to KiCad project format.
- If not specified and no file argument is provided, the build will use board files found by glob patterns or fall back to `mainEntrypoint`.
...
- `kicadLibraryName` (`string`): Name for the generated KiCad library.
...
- `glbs` (`boolean`): Enable GLB 3D model outputs for each circuit in build.
...
Controls rendering options for PCB images generated by `tsci export` (`pcb-svg` or `pcb-png`), `tsci snapshot`, and `tsci build` (when using `--pcb-svgs`, `--pcb-png`, or `--preview-images`).
...
- `xRayNets` (`string[]`): Exact net/trace names, trace display names, or connected Circuit JSON element IDs to X-Ray. An empty or omitted array keeps normal rendering.
- `hiddenLayerOpacity` (`number`): Opacity of unselected copper during X-Ray, from `0` to `1`. Defaults to `0.2`.
- `layer` (`"top" | "bottom"`): Layer to render normally, or frontmost layer during X-Ray.

For example, use the same two-net X-Ray view for PCB exports, snapshots, and build previews:
...
Selected copper and associated drills render fully opaque across layers. Other
copper is dimmed; non-copper layers (including courtyards and notes) and unrelated
drills are hidden. Explicit CLI selection, opacity, and layer options override
these defaults; CLI net selectors replace the configured list.
See [X-Ray PCB exports](../../command-line/tsci-export.md#x-ray-pcb-nets).

**How It Works:**

- Applied to `tsci export -f pcb-svg` and `tsci export -f pcb-png`.
- Applied when generating `pcb.svg` or `pcb.png` via `tsci snapshot`, `tsci build --pcb-svgs`, `tsci build --pcb-png`, and `tsci build --preview-images`.
- Has no effect on schematic or 3D outputs.
