...
| `useCloudAutorouting` | [Enable cloud autorouting for CLI builds, exports, and snapshots.](#usecloudautorouting) |
...
Enables cloud parallelism for Pipeline9 autorouting when running `tsci build`, `tsci export`, or `tsci snapshot`, including builds that use workers.
...
- When omitted or set to `false`, routing stays local.
- Cloud autorouting applies to Pipeline9 at the default `1x` effort level. Explicit alternative pipelines and other effort levels retain their local solver.
- The CLI maps this setting to [`platformConfig.useCloudAutorouter`](../running-tscircuit/platform-configuration.md#engines-and-routing). An explicit `platformConfig.useCloudAutorouter` in `tscircuit.config.ts` or `tscircuit.config.js` takes precedence.
...
Controls rendering options for PCB images generated by `tsci snapshot` and `tsci build` (when using `--pcb-svgs`, `--pcb-png`, or `--preview-images`).
