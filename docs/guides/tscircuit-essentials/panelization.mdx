...
Use [`<panel />`](../../elements/panel.mdx) to create a manufacturing panel that contains multiple boards.
Use [`<subpanel />`](../../elements/subpanel.mdx) to group boards inside a panel for nested layout and
...
- Each panel/subpanel must contain at least one `<board>`, directly or through nesting
...
<CircuitPreview
  ...
  code={`
export default () => (
  <panel layoutMode="none" width="100mm" height="100mm" pcbX="0mm" pcbY="0mm">
...
- `cellWidth`, `cellHeight` to set minimum grid cell sizes
...
Tab routing is disabled by default (`panelizationMethod="none"`). Set
`mouseBites={false}` to generate tabs without perforation holes.
