...
- `DTR` drives Q1's base through R3 and Q2's collector.
- `RTS` drives Q2's base through R4 and Q1's emitter.
- Q1's collector pulls `EN` low.
- Q2's emitter pulls `IO0` low.
...
```bash
bunx tsci build esp32-wifi-test-board.circuit.tsx --ignore-errors
bunx tsci snapshot esp32-wifi-test-board.circuit.tsx
bun test esp32-wifi-test-board.test.ts
```

The build and snapshot are verification tools, not a fabrication sign-off.
Before ordering, route and inspect the PCB, verify the USB-C footprint and
antenna keepout, run the repository's DRC checks, and confirm the selected JLC
