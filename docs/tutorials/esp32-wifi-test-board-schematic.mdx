...
This tutorial builds a schematic-first ESP32 WiFi test board. It is a complete
reference design for bringing up USB power, USB data, UART programming, and the
ESP32 module. The source uses repository-supported tscircuit components and
JLCPCB footprints, but the generated PCB preview is illustrative: review
routing, courtyard, antenna keepout, and assembly details before treating it as
...
The source also includes the exact passive and button part numbers, placement
coordinates, and all USB data nets. The checked-in schematic and PCB SVGs are
...
```bash
bunx tsci build esp32-wifi-test-board.circuit.tsx
bunx tsci snapshot esp32-wifi-test-board.circuit.tsx
bun test esp32-wifi-test-board.test.ts
```

The build command runs the repository autorouter and fails on placement or
routing errors; it does not suppress DRC. The snapshots are deterministic
previews, not a fabrication sign-off. Before ordering, inspect the PCB, verify
the USB-C footprint and antenna keepout, and confirm the selected JLC parts are
currently in stock.
