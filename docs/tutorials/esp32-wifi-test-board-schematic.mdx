...
const DeterministicTrace = (props: { from: string; to: string }) => (
  <trace
    {...props}
    name={["trace", props.from, props.to].join("_").replace(/[^a-zA-Z0-9_]/g, "_")}
  />
)
...
DRC. The source gives traces deterministic names, declares logical pin labels,
and resolves the exact supplier footprint data locally. The repeatable check
runs twenty clean schematic builds and twenty `snapshot --ci` comparisons,
hashes the declared SVG, rejects trailing whitespace, and requires identical
