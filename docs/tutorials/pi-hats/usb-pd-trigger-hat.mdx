...
Typical PD trigger boards expose a small selection table like this:

| Selector | Target voltage |
| --- | --- |
| `00` | 5V |
| `01` | 9V |
| `10` | 12V |
...
The exact codes depend on the controller family, so treat the table as a
bring-up pattern and map it to the chip you actually choose.
...
If your controller uses different selection pins, swap the net names in the
example so the documentation matches the exact module you are building.
