...
flowchart TD
    ...
    %% Checks to Core
    checks[tscircuit/checks] --> core[tscircuit/core]
    ...
    %% Runframe to circuitjson.com
    runframe --> circuitjsoncom[tscircuit/circuitjson.com]
    
    %% CLI to Tscircuit
    cli --> tscircuit[tscircuit/tscircuit]

    %% PnP converter supplies the CLI development dependency and tscircuit runtime dependency
    pnp[circuit-json-to-pnp-csv] --> cli
    pnp --> tscircuit
...
`circuit-json-to-pnp-csv` releases dispatch updates to both `tscircuit/cli` and
`tscircuit/tscircuit`. The CLI uses the converter as a development dependency and
leaves it external in its build; the `tscircuit` package supplies it at runtime.
When the CLI adopts a new converter API, publish the corresponding `tscircuit`
dependency update before merging the CLI change. Updating only the CLI's
development dependency does not update the converter in an installed CLI.
